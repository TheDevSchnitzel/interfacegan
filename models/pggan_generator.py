...
class PGGANGenerator(BaseGenerator):
    ...
    # Constructor call back to the BaseGenerator Class - Initializing the generator config 
    def __init__(self, model_name, logger=None):
      super().__init__(model_name, logger)
      assert self.gan_type == 'pggan'


    # Build the PGGAN Model
    def build(self):
    ...
    # This method is used to convert a TensorFlow(TF) based model into a pytorch based model
    def convert_tf_model(self, test_num=10):
        ...
        for pth_var_name, tf_var_name in self.model.pth_to_tf_var_mapping.items():
            ...
            # In case we do not have a mapping for the TF variable, just ignore it
            if tf_var_name not in tf_vars:
                self.logger.debug(f'Variable `{tf_var_name}` does not exist in tensorflow model.')
                continue

            # Begin converting non-special cases, switch from numpy to torch
    ...
    # Draw random normal-distributed samples
    def sample(self, num):
        assert num > 0
    ...
    # Preprocess latent codes by norming
    def preprocess(self, latent_codes):
        ...
        # Reshape array according to latent space dimensions => n vectory of latent_space_dim dimension
    ...
    # Generate the images according to the latent codes
    def synthesize(self, latent_codes):

        # Stop if the latent codes are delivered in a wrong format
        if not isinstance(latent_codes, np.ndarray):
            raise ValueError(f'Latent codes should be with type `numpy.ndarray`!')

        latent_codes_shape = latent_codes.shape

        # Stop if the latent codes array dimensions mismatch
        ...
        # Finally, create a latent codes tensor
        zs = torch.from_numpy(latent_codes).type(torch.FloatTensor)

        # Push the tensor to the executing device (CPU / GPU)
        zs = zs.to(self.run_device)

        # Execute the model on the latent tensor
