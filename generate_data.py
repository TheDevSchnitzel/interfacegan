...
def parse_args():
    ...
    parser.add_argument('-i', '--latent_codes_path', type=str, default='', help='If specified, will load latent codes from given path instead of randomly sampling. (optional)')
    parser.add_argument('-n', '--num', type=int, default=1, help='Number of images to generate. This field will be ignored if `latent_codes_path` is specified. (default: 1)')
    ...
    parser.add_argument('-S', '--generate_style', action='store_true', help='If specified, will generate layer-wise style codes in Style GAN. (default: do not generate styles)')
    parser.add_argument('-I', '--generate_image', action='store_false', help='If specified, will skip generating images in Style GAN. (default: generate images)')
...
def main():
    ...
    # Selecting a GAN and creating the according generator
    ...
    # Here we do the latent code preprocessing, either they have been provided as files or they are generated on the fly
    ...
    # We use the loaded latent codes to actually generate images utilizing the selected GAN
    # Therrefore, we first get a batch of codes and iteratively generate images
    logger.info(f'Generating {total_num} samples.')
    results = defaultdict(list)
    pbar = tqdm(total=total_num, leave=False)

    # For each latent code generate an image
    for latent_codes_batch in model.get_batch_inputs(latent_codes):
        ...
        # Save each generated image as a file
    ...
    # Save the additionally generated informations about the latent space 
