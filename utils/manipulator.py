...
def train_boundary(latent_codes,
                   scores, # These scores have to be chosen manually by humans
    ...
    # Remaining set. Will be used for after-validation predictions
    ...
    logger.info(f'  Remaining: {remaining_positive_num} positive, {remaining_negative_num} negative.')


    # Train the SVM with the preselected training data
    ...
    # Run the classifier on the validation set
    if val_num:
    ...
    # Run the classifier in the remaining set
    if remaining_num:
    ...
    # Get the classifiers coeffiecients and return them normalized
...
def project_boundary(primal, *args):
    ...
    if len(args) == 1:
        ...
        for cond_boundary in args:
            assert (len(cond_boundary.shape) == 2 and cond_boundary.shape[0] == 1 and cond_boundary.shape[1] == primal.shape[1])
